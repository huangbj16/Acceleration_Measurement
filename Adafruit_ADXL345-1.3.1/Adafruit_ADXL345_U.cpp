...
bool Adafruit_ADXL345_Unified::readXYZ(uint8_t reg, uint16_t* data) {
  uint8_t buffer[6] = {i2c_dev ? reg : reg | 0x80 | 0x40, 0, 0, 0, 0, 0};
  if (i2c_dev) {
    i2c_dev->write(buffer, 1);
    i2c_dev->read(buffer, 6);
  } else {
    spi_dev->write_then_read(buffer, 1, buffer, 6);
  }
...
Adafruit_ADXL345_Unified::Adafruit_ADXL345_Unified(uint8_t clock, uint8_t miso,
  ...
  _sensorID = sensorID;
  _range = ADXL345_RANGE_2_G;
  spi_dev = new Adafruit_SPIDevice(cs, clock, miso, mosi, 2000000,
...
bool Adafruit_ADXL345_Unified::begin(uint8_t i2caddr) {
  if (spi_dev == NULL) {
    if (i2c_dev)
      delete i2c_dev;
    i2c_dev = new Adafruit_I2CDevice(i2caddr, &Wire1);
    if (!i2c_dev->begin())
      return false;
  } else {
    if (!spi_dev->begin())
...
bool Adafruit_ADXL345_Unified::getEventXYZ(sensors_event_t *event) {
  /* Clear the event */
  memset(event, 0, sizeof(sensors_event_t));
  ...
  event->timestamp = 0;
  uint16_t data[3];
  if(readXYZ(ADXL345_REG_DATAX0, data)){
    event->acceleration.x =
        int16_t(data[0]) * ADXL345_MG2G_MULTIPLIER * SENSORS_GRAVITY_STANDARD;
    event->acceleration.y =
        int16_t(data[1]) * ADXL345_MG2G_MULTIPLIER * SENSORS_GRAVITY_STANDARD;
    event->acceleration.z =
        int16_t(data[2]) * ADXL345_MG2G_MULTIPLIER * SENSORS_GRAVITY_STANDARD;
  }
  return true;
}
